...
export default {
  ...
  methods: {
    ...
    getJobList(page, pageSize) {
      this.loading = true;
      axios.post("/production/selectPage",{
          page: page,
          pageSize: pageSize,
        })
        .then((res) => {
    ...
    doOptions(jobId, value) {
      ...
      if (value === 3) {
        ...
        this.$confirm("确定要发布吗?", "提示", {
          ...
          axios({
            method: "POST",
            url: "/production/approval/issueTask",
            data: {
              jobId: jobId,
            },
          })
      ...
      if (value === 4) {
        this.$confirm(
          ...
          axios.post("/production/taskExecute/discontinueTask",{"jobId":jobId})
            .then((res) => {
              this.getJobList(this.currentPage, this.pageSize);
            })
            .catch((err) => {
