<template>
  <div>
    <el-table :data="users" @selection-change="selectChange" style="width: 100%;font-size:12px">
      ...
      <el-table-column label="操作" fixed="right" min-width="15%">
        <template slot-scope="scope" style="position: relative">
          <el-link type="primary" v-for="(ao,i) in adminOptions" :key="i" style="margin-left:10px"
            @click="doOptions(scope.row.jobId,i)">{{ao}}</el-link>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getUsers(page, pageSize) {
        ...
        axios({
            method: "POST",
            url: "/test-4/approval/selectByJobState",
            data: {
              jobId: jobId,
            },
          })
